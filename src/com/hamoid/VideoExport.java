 ...
 *
 * @author Quinton Ashley http://qashto.com
 * @modified 02/11/2018
 * @version 1.0.2
 */
...
public class VideoExport {
	...
	/**
	 * Set the video codec to use. Optional.
	 *
	 * @param codec
	 * The video codec to use.The default is libx265.
	 */
	public void setVideoCodec(String vcodec) {
		ffmpegVideoCodec = vcodec;
	}
	 ...
	 *
	 * @param crf
	 * A value between 0 (high quality, lossless)
	 * and 51(high compression). Default is 0.
	 */
	public void setQuality(int crf) {
		...
		ffmpegCrfQuality = crf;
	}

	/**
	 * Set the quality of the color. Optional.
	 *
	 * @param depth
	 * The color bit depth.The default is 10.
	 */
	public void setColorBitDepth(int depth) {
		if (depth < 8 || depth > 12) {
			System.err.println("Invalid bit depth, valid options are 8, 10, and 12");
			return;
		}
		if (depth == 8) {
	...
	public void saveFrame() {
		if (img != null && img.width > 0) {
			...
			if (pixelsByte == null) {
				pixelsByte = new byte[img.width * parent.pixelDensity *
				                      img.height * parent.pixelDensity * 3];
			}
	...
	private void startFfmpeg(String executable) {
		...
		processBuilder = new ProcessBuilder(executable, "-y", "-f", "rawvideo",
				"-vcodec", "rawvideo", "-s",
				(img.width*parent.pixelDensity) + "x" + (img.height*parent.pixelDensity),
				"-pix_fmt", "rgb24", "-r", "" + ffmpegFrameRate, "-i", "-",
				"-an", "-c:v", ffmpegVideoCodec, "-pix_fmt", ffmpegPixelFormat, "-crf",
				"" + ffmpegCrfQuality, ((ffmpegVideoCodec.contains("vp9"))? "-b:v": ""), ((ffmpegVideoCodec.contains("vp9"))? "0": ""),  "-metadata",
